...
private:
    ...
    static bool HaveExtraMinus(const string& word) {
        return (word[1] == '-' || (word.size() == 1 && word[0] == '-')); 
    }
};
/* по минусу в конце слова: изначально в голове подумал, что в конце слова не должно быть "-". 
С первого ревью понял, что ошибся. Когда переписывал, скопировал не тот код. Поторопился.
Хотел вместо word[word.size() - 1] использовать  (word.size() == 1 && word[0] == '-').*/
